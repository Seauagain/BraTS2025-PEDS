...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.plans_dict= plans

        if not self.plans_dict['std_gamma'] == "":
            self.output_folder_base += f"__std_gamma{self.plans_dict['std_gamma']}"
            
        # print("self.output_folder_base: ", self.output_folder_base)
        
        # print("self.plans_manager.plans_name: ", self.plans['std_gamma'])
        # print("plans: ", plans)
        # exit(0)
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if isinstance(gamma, (int, float) ):
                ...
                self.print_to_log_file(f"The distribution of model weights have been changed with std rate = {gamma}!!!")
            
            self.print_to_log_file(f"self.output_folder_base: {self.output_folder_base}")
    ...
    def _do_i_compile(self):
        ...
        if self.device == torch.device('mps'):
            if 'nnUNet_compile' in os.environ.keys() and os.environ['nnUNet_compile'].lower() in ('true', '1', 't'):
                self.print_to_log_file("INFO: torcfh.compile disabled because of unsupported mps device")
            return False
