...
def run_training_entry():
    ...
    def flexible_number(value):
        """自定义类型转换函数，支持空字符串和数值输入"""
        if value == "":  # 默认空字符串直接返回
            return value
        try:
            return float(value)  # 兼容整数和浮点数输入
        except ValueError:
            raise argparse.ArgumentTypeError(f"'{value}' 必须是整数或浮点数")

    parser.add_argument(
        '-gamma', 
        type=flexible_number,  # 绑定自定义转换函数[3,6](@ref)
        required=False,
        default="",  # 
        help="the std rate gamma of initial model weights, default ''."
    )
    ...
    # 如果提供了 gamma 参数且不为空字符串，则修改 plans 文件
    if args.gamma != "":
        plans = modify_std_gamma(plans_file, float(args.gamma))
    else:
        plans = modify_std_gamma(plans_file, args.gamma)
...
def modify_std_gamma(plans_file: str, gamma_value: float):
    """
    修改 plans 文件中的 std_gamma 值
    """
    from batchgenerators.utilities.file_and_folder_operations import join, isdir, isfile, maybe_mkdir_p, load_json, save_json

    if not isfile(plans_file):
        raise FileNotFoundError(f"Plans 文件不存在: {plans_file}")
    # 加载 plans 文件
    plans = load_json(plans_file)
    # 修改 std_gamma 值
    if "std_gamma" in plans:
        print(f"将 std_gamma 从 {plans['std_gamma']} 修改为 {gamma_value}")
    else:
        print(f"添加 std_gamma 值: {gamma_value}")
    
    plans["std_gamma"] = gamma_value
    # 保存修改后的 plans 文件
    save_json(plans, plans_file, sort_keys=False)
    print(f"已成功更新 plans 文件: {plans_file}")
