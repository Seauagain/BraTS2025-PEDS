...
class LabelManager(object):
    ...
    def convert_probabilities_to_segmentation(self, predicted_probabilities: Union[np.ndarray, torch.Tensor]) -> \
        ...
        if self.has_regions:
            ...
            for i, c in enumerate(self.regions_class_order):  #  原始代码
                segmentation[predicted_probabilities[i] > 0.5] = c   


            # 处理每个区域
            # print("=========================utilities/label_handling/label_handling.py: 自定义后处理噢！！！======================" )
            # threshold = 0.5
            # regions_class_order = self.regions_class_order

            # c6_mask = (predicted_probabilities[5] > threshold) & (predicted_probabilities[4] <= threshold)
            # segmentation[c6_mask] = 4

            # for i, c in enumerate(regions_class_order):
            #     if i == 5:  # 第6类对应标签4
            #         # 直接将概率>阈值的像素设置为标签4
            #         # segmentation[predicted_probabilities[i] > threshold] = 4
            #         pass 
                    
            #     elif i == 4:  # 第5类，需要特殊处理
            #         pass 
            #         # # 只看前3个通道(0,1,2)中概率最大的
            #         # max_prob_channel = np.argmax(predicted_probabilities[:3], axis=0)
            #         # max_prob_values = np.max(predicted_probabilities[:3], axis=0)
                    
            #         # # 创建掩码：最大概率>阈值的位置
            #         # valid_mask = max_prob_values > threshold
                    
            #         # # 对于有效位置，根据最大概率通道设置标签
            #         # for j in range(3):  # 只考虑前3个通道 channel-wise 
            #         #     channel_mask = (max_prob_channel == j) & valid_mask
            #         #     segmentation[channel_mask] = regions_class_order[j]  # 对应1,2,3
                    
            #         # 概率小于阈值的位置保持为0（背景）
                    
            #     else:  # 其他区域按原始规则处理 
            #         if i < 4:  # 只处理前4个区域，确保标签不超过4
            #             target_label = min(c, 4)  # 确保标签不超过4
            #             segmentation[predicted_probabilities[i] > threshold] = target_label
            
            # # 确保分割掩码只包含0,1,2,3,4
            # segmentation = np.clip(segmentation, 0, 4)
